      ...
      <div className="absolute bottom-4 left-4 z-10 bg-white/95 backdrop-blur-md p-3.5 rounded-3xl shadow-md border border-slate-200 text-xs sm:text-sm hidden sm:block pointer-events-none">
        ...
        {currentLevel === 1 ? (
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2">
            <div className="flex items-center gap-2">
              <span className="w-3.5 h-3.5 rounded-full bg-slate-300 border border-slate-400"></span>
              <span className="text-slate-600 font-semibold">Chưa chọn</span>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              <span className="w-3.5 h-3.5 rounded-full bg-amber-500 border border-amber-600"></span>
              <span className="text-slate-800 font-black">Đã ghép đúng</span>
            </div>
            <div className="flex items-center gap-2">
        ...
        <g id="vietnam-provinces-layer" filter="url(#map-shadow)">
          {provincesWithGeo.map((province) => {
            ...
            if (currentLevel === 1) {
              ...
              isCompleted = completedGroupIds.includes(province.groupId);
          ...
          {provincesWithGeo.map((province) => {
            const isHighlighted =
              currentLevel === 1
                ...
                : selectedProvinceIds.includes(province.id) || completedGroupIds.includes(province.groupId);
            ...
            return (
              <text
                ...
                onMouseEnter={() => handleHover(province)}
